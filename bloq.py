...
# Función para bloquear sitios web
def block_websites_remote():
    ...
    # Comandos para agregar las entradas al archivo /etc/hosts
...
# Función para desbloquear sitios web
def unblock_websites_remote():
    ...
    # Comandos para eliminar las entradas del archivo /etc/hosts en macOS
    commands = ["sudo sed -i '' '/0.0.0.0 {}[[:space:]]*$/d' /etc/hosts".format(domain.strip()) for domain in domains]
    commands += ["sudo sed -i '' '/0.0.0.0 www.{}[[:space:]]*$/d' /etc/hosts".format(domain.strip()) for domain in domains]
    # Vaciar caché DNS en macOS
    commands.append("sudo dscacheutil -flushcache")
    commands.append("sudo killall -HUP mDNSResponder")
    
    command = " && ".join(commands)
    
    output, error = execute_remote_command(host, username, password, command)
    if error:
        messagebox.showerror("Error", f"Error al desbloquear sitios web: {error}")
    else:
        messagebox.showinfo("Éxito", f"Acceso restaurado para los dominios: {', '.join(domains)}")

# Habilitar los botones solo si todos los campos están llenos
...
root = tk.Tk()
root.title("Bloqueo/Desbloqueo de Páginas Web")
...
tk.Label(root, text="Páginas Web a bloquear/desbloquear (separadas por coma):").grid(row=3, column=0, padx=10, pady=5)
